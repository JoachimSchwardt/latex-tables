...
class HeatTable:
    ...
    def __validate_input(self):
        ...
        if self.cells.ndim == 1:
            ...
            if np.sum(self.cells.shape != 1) == 2:
                old_shape = self.cells.shape
                self.cells = np.squeeze(self.cells)
                print(f"Warning: 'cells' is is of shape {old_shape} "
                      + f"which will be converted to shape {self.cells.shape}!")
            else:
